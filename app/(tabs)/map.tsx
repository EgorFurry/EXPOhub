...
const PAVILION_PLANS: Record<string, string> = {
  '9':  'https://via.placeholder.com/2000x1400/1a1a2e/C41230?text=Pavilion+9',
  '10': 'https://via.placeholder.com/2000x1400/1a1a2e/C41230?text=Pavilion+10',
  '11': 'https://via.placeholder.com/2000x1400/1a1a2e/C41230?text=Pavilion+11',
};
...
function buildLeafletHTML(booths: Booth[], pavilion: string, accentColor: string): string {
  const planUrl = PAVILION_PLANS[pavilion] ?? PAVILION_PLANS['11'];
...
const bounds = [[0,0],[1400,2000]];
L.imageOverlay('${planUrl}', bounds).addTo(map);
...
function renderBooths(booths) {
  ...
  booths.forEach(booth => {
    ...
    const color = dimmed ? '#555' : ACCENT;
    const opacity = dimmed ? 0.2 : 1;
    if (booth.polygon && booth.polygon.length > 2) {
      const latlngs = booth.polygon.map(p => [p.y, p.x]);
      const poly = L.polygon(latlngs, { color, fillColor:color, fillOpacity:dimmed?0.05:0.25, weight:1.5, opacity }).addTo(map);
      if (!dimmed) { poly.bindPopup(buildPopup(booth)); poly.on('click', () => { poly.openPopup(); notifySelected(booth.id); }); }
      markers.push(poly);
    } else {
      const x = booth.cx ?? (200 + Math.random()*1600);
      const y = booth.cy ?? (200 + Math.random()*1000);
      const icon = L.divIcon({ html: '<div style="background:'+color+';border:2px solid #fff;border-radius:6px;padding:3px 6px;font-size:10px;font-weight:700;color:#fff;white-space:nowrap;opacity:'+opacity+';">'+( booth.number||booth.name.slice(0,8))+'</div>', className:'', iconAnchor:[0,0] });
      const marker = L.marker([y,x],{icon}).addTo(map);
      if (!dimmed) { marker.bindPopup(buildPopup(booth)); marker.on('click', () => { marker.openPopup(); notifySelected(booth.id); }); }
      markers.push(marker);
    }
  });
}
...
function buildPopup(booth) {
  ...
  return '<div class="popup-name">'+booth.name+'</div><div class="popup-meta">Стенд '+booth.number+' · '+booth.hall+'</div>'+(tagsHtml?'<div class="popup-tags">'+tagsHtml+'</div>':'')+'<div class="popup-btns"><button class="popup-btn popup-btn-secondary" onclick="showRoute(\''+booth.id+'\')">🧭 Путь</button><button class="popup-btn popup-btn-primary" onclick="openBooth(\''+booth.id+'\')">Открыть →</button></div>';
}
...
function showRoute(boothId) {
  ...
  const tx = booth.cx ?? (booth.polygon?.[0]?.x ?? 1000);
  const ty = booth.cy ?? (booth.polygon?.[0]?.y ?? 700);
  routeLine = L.polyline([[center.lat,center.lng],[ty,tx]], { color:ACCENT, weight:3, dashArray:'8,8', opacity:0.9 }).addTo(map);
  map.flyTo([ty,tx], 1, { duration:0.8 });
...
function searchBooth(query) {
  ...
  if (found) {
    const x = found.cx ?? 1000; const y = found.cy ?? 700;
...
function notifySelected(boothId) { window.ReactNativeWebView.postMessage(JSON.stringify({ type:'BOOTH_SELECTED', boothId })); }
function openBooth(boothId) { window.ReactNativeWebView.postMessage(JSON.stringify({ type:'OPEN_BOOTH', boothId })); }

window.addEventListener('message', (e) => {
  try {
    const msg = JSON.parse(e.data);
    if (msg.type==='SET_BOOTHS') renderBooths(msg.booths);
    if (msg.type==='FLY_TO') map.flyTo([msg.y,msg.x], msg.zoom||1);
...
export default function MapScreen() {
  ...
  return (
    <View style={styles.root}>
      ...
      <WebView ref={webViewRef} style={styles.webview} originWhitelist={['*']} source={{ html: htmlContent }}
        onMessage={handleWebViewMessage} javaScriptEnabled domStorageEnabled
        scrollEnabled={false} bounces={false} overScrollMode="never" mixedContentMode="always" allowFileAccess />
